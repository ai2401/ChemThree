...
import { GLTFExporter, RoomEnvironment } from 'three-stdlib';
...
type EnvMode = 'room' | 'hdr';

const EnvController: React.FC<{ mode: EnvMode }> = ({ mode }) => {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // RoomEnvironment (which extends Scene) does not have a dispose method.
      // We only dispose the generated texture/target and the generator.
      pmremGenerator.dispose();
    };
  }, [mode, gl, scene]);

  // If HDR mode, render the Environment component which handles loading and applying the HDR texture
  if (mode === 'hdr') {
    return <Environment files="studio_small_03_1k.hdr" blur={0.8} />;
  }
...
const App: React.FC = () => {
  ...
  const [envMode, setEnvMode] = useState<EnvMode>('room');
  
  // Reference to the group containing the actual molecule content
  ...
  const handleExport = () => {
    ...
    setIsExporting(true);
    // Explicitly cast to any to resolve TS error: "Only a void function can be called with the 'new' keyword"
    const exporter = new (GLTFExporter as any)();
    
    // Parse the input (sceneRef.current) to GLTF
    exporter.parse(
      sceneRef.current,
      (gltf: any) => {
        ...
        link.href = url;
        link.download = `chemart-${sceneName}-${timestamp}.glb`;
      ...
      // Options
      {
        binary: true, // Export as .glb (binary) which is self-contained
      }
    );
  };

  const toggleEnvMode = () => {
    setEnvMode(prev => prev === 'room' ? 'hdr' : 'room');
  };
    ...
    <div className="w-full h-screen relative bg-slate-900 overflow-hidden selection:bg-indigo-500 selection:text-white">
      ...
      <div className="absolute inset-0 z-0">
        <Canvas 
          ...
          gl={{ antialias: false, preserveDrawingBuffer: true, toneMapping: THREE.ACESFilmicToneMapping }}
        >
          <color attach="background" args={['#0f172a']} />
          
          <Suspense fallback={null}>
            {/* Environment Controller */}
            <EnvController mode={envMode} />
            
            <StarsWrp />
            
            {/* Main Content wrapped in a group for Export reference */}
            <group ref={sceneRef} position={[0, 0, 0]}>
               {renderActiveScene()}
            </group>

            {/* Lighting for that "Nature" Look - Always keep basic lights to ensure structure is visible even if Env changes */}
          ...
          <div className="flex flex-col items-end gap-2">
            <div className="flex gap-2 items-center">
               ...
               <button 
                 ...
                 className="flex items-center gap-2 bg-white/5 backdrop-blur-md border border-white/10 rounded-full px-3 py-1 text-xs text-slate-300 font-sans hover:bg-white/10 transition-colors hover:text-white"
                 title={envMode === 'room' ? t.envSynthetic : t.envHdr}
               >
                 {envMode === 'room' ? <SunIcon /> : <ImageIcon />}
                 <span className="hidden sm:inline">{envMode === 'room' ? t.envSynthetic : t.envHdr}</span>
               </button>
               ...
               <button 
                 ...
                 className="flex items-center gap-2 bg-white/5 backdrop-blur-md border border-white/10 rounded-full px-3 py-1 text-xs text-slate-300 font-sans hover:bg-white/10 transition-colors hover:text-white disabled:opacity-50"
                 title={t.exportBtn}
               >
                 <BoxIcon />
                 <span className="hidden sm:inline">{isExporting ? '...' : t.exportBtn}</span>
               </button>
...
// Wrapper for stars to avoid re-rendering stars on env switch if possible, though suspense handles it.
const StarsWrp = React.memo(() => <Stars radius={100} depth={50} count={5000} factor={4} saturation={0} fade speed={1} />);
